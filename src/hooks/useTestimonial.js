...
const useTestimonial = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setDataTestimonial(response.data.data);
    } catch (error) {
  ...
  const postData = async (data, token) => {
    setLoading(true);
    try {
      const response = await axios.post(`${API_URL}/api/testimonials`, data, {
        headers: {
  ...
  const updateData = async (id, data, token) => {
    setLoading(true);
    try {
      const response = await axios.put(`${API_URL}/api/testimonials/${id}`, data, {
        headers: {
  ...
  return { dataTestimonial, error, loading, postData, updateData, deleteData };
};
