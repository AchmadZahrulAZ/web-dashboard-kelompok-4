...
const useTeams = () => {
  ...
  const postData = async (data, token) => {
    setLoading(true);
    try {
      const response = await axios.post(`${API_URL}/api/team`, data, {
        headers: {
  ...
  const updateData = async (id, data, token) => {
    setLoading(true);
    try {
      const response = await axios.put(`${API_URL}/api/team/${id}`, data, {
        headers: {
  ...
  return { dataTeams, error, loading, postData, updateData, deleteData };
};
